...
const DatabaseView = ({ database, setDatabase, showToast }) => {
    ...
    const renderContent = () => {
        if (database.length === 0) {
            return React.createElement('div', {className: 'placeholder'}, '데이터베이스가 비어 있습니다. 첫 패키지를 계산하고 저장해보세요!');
        }
        if (filteredAndSortedDatabase.length === 0) {
            return React.createElement('div', {className: 'placeholder'} as any, '조건에 맞는 데이터가 없습니다.');
        }
                            ...
                            return React.createElement('div', { key: pkg.id, className: 'related-package-item' },
                                ...
                                React.createElement('div', { className: 'related-package-content' },
                                    React.createElement('div', { className: 'result-item' },
                                        ...
                                        React.createElement('span', { className: efficiencyBadgeClass }, formatEfficiency(pkg.efficiency))
                                    ),
                                    React.createElement('p', { className: 'items-cell' },
                                        `${pkg.saleDate} | `,
